...
function RouteComponent() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        const validCommitments = value.fixedCommitmentsJson.commitments.filter(
  ...
  const handleSaveEdit = (newPlanData) => {
  ...
  const handleViewFull = async () => {
    ...
    const planId = await save();
    if (planId) {
      // Navigate to dashboard page with planId
      navigate({ to: "/dashboard" });
    } else {
      // If save failed (no planId), still navigate to dashboard
      navigate({ to: "/dashboard" });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main
        className={`container mx-auto px-4 py-8 ${hasGenerated ? "lg:flex lg:gap-8" : "max-w-4xl"}`}
      >
          ...
          >
                ...
                <form.Field name="taskComplexity">
                  {(field) => (
                    <RadioGroup
                      value={field.state.value}
                      onValueChange={(value) =>
                        field.handleChange(value as "Simple" | "Balanced" | "Ambitious")
                      }
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>What areas do you want to focus on this month?</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  {(field) => (
                    <div className="space-y-3">
                      <RadioGroup
                        value={field.state.value}
                        onValueChange={(value) =>
                          field.handleChange(value as "Work" | "Rest" | "Mixed")
                        }
                        className="grid grid-cols-1 md:grid-cols-3 gap-4"
                      >
                        <div className="flex items-center space-x-2">
                  ...
                  {(field) => (
                    <>
                      {field.state.value.map((_, i) => (
                        <div
                          key={i}
                          className="grid grid-cols-1 md:grid-cols-[140px,120px,120px,1fr,40px] gap-3 p-4 border rounded-lg relative"
                        >
                          <div>
        ...
        {isGenerating && (
          <div className="lg:w-100 lg:sticky lg:top-8 lg:h-fit">
            <ParsingStatus isLoading={true} aiResponse={null} error={undefined} />
          </div>
        )}
