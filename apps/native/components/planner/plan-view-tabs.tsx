...
import { Card, Button, Skeleton, Tabs } from "heroui-native";
  ...
  return (
    <Container>
      <Tabs value={activeTab} onValueChange={setActiveTab} variant="line" className="flex-1">
        <Tabs.List className="border-b border-divider px-4">
          <Tabs.ScrollView
          ...
          >
            ...
            <Tabs.Trigger value="tasks">
              <Tabs.Label>Tasks ({allTasks.length})</Tabs.Label>
            </Tabs.Trigger>
            <Tabs.Trigger value="goals">
              <Tabs.Label>Goals ({allGoals.length})</Tabs.Label>
            </Tabs.Trigger>
          </Tabs.ScrollView>
        </Tabs.List>
        ...
        >
          ...
          <Tabs.Content value="tasks">
            <PlanHeader planInfo={planInfo} structuredData={structuredData} />
            <Card className="m-4 p-4">
              <Text className="text-lg font-semibold text-foreground mb-4">All Tasks</Text>
              {allTasks.map((task) => (
                <View
                  key={task.id}
                  className="flex-row items-center gap-3 py-3 border-b border-border last:border-0"
                >
                  <TouchableOpacity
                    className={`w-6 h-6 rounded-full border-2 items-center justify-center ${
                      task.isCompleted ? "bg-primary border-primary" : "border-muted-foreground"
          ...
          <Tabs.Content value="goals">
            <PlanHeader planInfo={planInfo} structuredData={structuredData} />
            <Card className="m-4 p-4">
              <Text className="text-lg font-semibold text-foreground mb-4">All Goals</Text>
              {allGoals.map((goal, i) => (
                <View
                  key={i}
                  className="flex-row items-start gap-3 py-2 border-b border-border last:border-0"
                >
