...
// Create MMKV instance for auth store (encrypted)
const authStorage = createMMKV({
  id: 'auth-storage',
  encryptionKey: 'auth-encryption-key-123', // In production, use secure key
});
  ...
  persist(
    ...
    }),
    {
      ...
      partialize: (state: AuthStore) => ({
        // Only persist these fields to MMKV
        // Authentication token and sensitive data are handled by expo-secure-store
        isAuthenticated: state.isAuthenticated,
        user: state.user,
        // Don't persist isLoading, error, or authLoaded - these should be fresh on app start
      }),
      onRehydrateStorage: () => (state?: AuthStore) => {
        // After rehydration, set authLoaded to true and check real auth status
        if (state) {
          state.authLoaded = false; // Will be set to true after checkAuthStatus
        }
      },
    }
  )
