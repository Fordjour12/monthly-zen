...
/**
     ...
     */
   export async function executeAIRequest<TInput, TOutput>(
    ...
    export async function regeneratePlan(
       ...
       try {
          ...
          try {
             ...
             if (suggestion && suggestion.applicationHistory && suggestion.applicationHistory.length > 0) {
                const latestExecution = suggestion.applicationHistory[suggestion.applicationHistory.length - 1];
                executionInsights = `
...
- If motivation was low: Add quick wins, milestone celebrations`;
          ...
          const response = await executeAIRequest<string, PlanSuggestionContent>({
             type: "plan",
...
**Output Format (JSON):**
{
  ...
  "success_metrics": ["Updated success metrics"]
}
...
**Regeneration Guidelines:**
1. Address the specific user concerns mentioned in the regeneration reason
2. Learn from previous execution insights and avoid repeating issues
3. Maintain the core goals but adjust approach, timeline, or breakdown
4. Consider workload balance and realistic time estimates
5. Provide more flexibility where previous plan was too rigid
6. Add buffer time for unexpected delays if time management was an issue
                ...
                if (scheduledEvent.success) {
                   ...
                   if (item.type === 'task') {
                      await taskQueries.createTask(userId, {
                         ...
                         dueDate: scheduledEvent.startTime,
...
/**
                ...
                try {
                   ...
                   for (const event of taskEvents) {
                      ...
                      const conflictingEvents = await calendarQueries.findOverlappingEvents(
                         userId,
                         newDateTime,
                         new Date(newDateTime.getTime() + (event.endTime - event.startTime))
                      );
                      ...
                      const updatedEvent = await calendarQueries.updateEvent(event.id, {
                         startTime: newDateTime,
                         endTime: new Date(newDateTime.getTime() + (event.endTime - event.startTime)),
                      });
                      ...
                      if (task) {
                         ...
                         for (const event of taskEvents) {
                            await calendarQueries.updateEvent(event.id, {
                               ...
                               endTime: new Date(pauseUntil.getTime() + (event.endTime - event.startTime)),
                            });
                         }
                   ...
                   if (resumeItem.type === "task") {
                      ...
                      if (task) {
                         ...
                         for (const event of taskEvents) {
                            await calendarQueries.updateEvent(event.id, {
                               startTime: now,
                               endTime: new Date(now.getTime() + (event.endTime - event.startTime)),
                            });
                         }
          ...
          if (modifications.bulkReschedule) {
             try {
                ...
                for (const event of eventsInRange) {
                   ...
                   const newStartTime = new Date(event.startTime + timeDifference);
                   const newEndTime = new Date(event.endTime + timeDifference);
             ...
             try {
                ...
                if (resolution.success) {
                   ...
                   resolutions.push({
                      ...
                      action: resolution.primaryAction,
     ...
     */
    async function prioritizeStrategy(events: any[], severity: string): Promise<any> {
...
/**
    ...
    export async function trackPlanExecution(
       ...
       try {
          ...
          const timeAnalysis = await analyzeTimePerformance(planId, completedTasks);
                  ...
                  if (Array.isArray(tasks)) {
                     tasks.forEach((taskTitle: string) => {
                        const classification: SuggestionItemClassification = {
                           title: taskTitle,
                           type: applyAs,
                           confidence: calculateConfidence(taskTitle, applyAs),
                           reasoning: generateReasoning(taskTitle, applyAs),
                           suggested_priority: inferPriority(taskTitle),
                           estimated_duration: "30 min",
                           due_date: calculateDueDate(weekIndex, day),
                           quick_win: isQuickWin(taskTitle),
                           long_term_build: isLongTermBuild(taskTitle),
                        };

if (applyAs === "recurring-task") {
                            classification.recurrence_rule = generateRecurrenceRule(taskTitle);
                         } else if (applyAs === "habit") {
                            classification.habit_potential = analyzeHabitPotential(taskTitle);
                            classification.suggested_frequency = classification.habit_potential.frequency;
                         }
...
/**
     ...
     */
    async function analyzeTimePerformance(planId: string, completedTasks: string[]): Promise<{
       onTimeRate: number;
       averageDelay: number;
       insights: string[];
    }> {
       ...
       try {
          ...
          for (const taskTitle of completedTasks) {
             ...
             const tasks = await taskQueries.findByUser(""); // We'll need userId here in real implementation
